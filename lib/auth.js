...
// · 令牌双通道：浏览器带 ops_session Cookie；机器客户端(采集端 harness)带
//   Authorization: Bearer <token>——同一套账号/会话，只是携带方式不同（「统一登录」）。
// · 单一管理员：新建用户恒 role='user'（应用层）；库层 ux_accounts_admin partial unique
//   index 兜底，任何路径都造不出第二个 admin（见 lib/db.js DDL_V2）。
// · 门禁是「全局」中间件（含 static），未登录访问页面→302 login.html，访问 /v1→401 JSON。
...
// ---- 全局门禁中间件 ----
// 白名单：探活、登录页、登录接口。其余：命中会话放行并挂 req.account；
// 未登录时 /v1 请求→401 JSON，页面请求→302 至 login.html。
