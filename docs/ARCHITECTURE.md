...
单 `index.html` + hash tab 六页(概览/趋势/**工作流**/**用户**/失败/实例)，vanilla ESM：`js/core/{api,fmt,charts,range}.js` + `js/features/{overview,trend,blueprints,users,failures,installs}.js`。图表用 vendored uPlot(挂 `window.uPlot`，经 `core/charts.js` 收口)。60s 自动刷新当前激活页，`document.hidden` 时暂停。

- **工作流页**：各工作流 runs/失败/活跃时长/中断/token，表头点击排序(复用 installs 纯前端排序范式)。
- **用户页**：Top 10 高频用户，metric 选择器(会话/run/token/时长/失败)切换即重查 `/stats/users/top?metric=`。
