...
- **零构建**：看板是 vanilla ESM + vendored 前端库（uPlot / ECharts / IBM Plex Mono），源码即产物。
...
单 `index.html` + hash tab 六页(概览/趋势/**工作流**/**用户**/失败/实例)，vanilla ESM：`js/core/{api,fmt,charts,echarts,range}.js` + `js/features/{overview,trend,blueprints,users,failures,installs}.js`。设计语言为「精密仪表盘」暗色(amber 信号色 + cyan 副色、IBM Plex Mono 等宽数字、载入错峰揭示)。两套 vendored 图表库各收口于 core：**uPlot**(`core/charts.js`，趋势时序)与 **ECharts**(`core/echarts.js`，排名/堆叠条形)。60s 自动刷新当前激活页，`document.hidden` 时暂停。
